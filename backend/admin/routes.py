...
async def get_audit_logs(
    ...
    try:
        ...
        # Note: DAL is async, so we keep this route async
        ...
        for row in raw_logs:
            ...
            logs.append(AuditLogEntry(
                ...
                timestamp=str(row.get('created_at', "") or row.get('timestamp', "")),
...
def list_users(
    ...
    Admin only. Sync route for SQLModel.
    """
    ...
    for user in users:
        ...
        session_result = session.execute(
            ...
            .where(Session.is_valid == True) # Check valid sessions preferred
